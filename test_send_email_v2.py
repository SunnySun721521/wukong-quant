...
def test_send_test_email():
    ...
    try:
        response = requests.post(f"{BASE_URL}/api/email/test", json=test_data, timeout=30)
    ...
    # 检查邮件发送日志
    print("\n4. 检查邮件发送日志...")
    response = requests.get(f"{BASE_URL}/api/email/logs", timeout=10)
    if response.status_code == 200:
        logs = response.json()
    ...
    # 检查邮件发送统计
    print("\n5. 检查邮件发送统计...")
    response = requests.get(f"{BASE_URL}/api/email/statistics", timeout=10)
    if response.status_code == 200:
        stats = response.json()
