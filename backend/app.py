...
# Render兼容模块 - 不影响本地运行
try:
    from render_compat import preload_render_data, is_render_environment
    RENDER_MODE = is_render_environment()
    if RENDER_MODE:
...
def save_backtest_pool(stocks):
    """保存回测股票池"""
    try:
        os.makedirs(os.path.dirname(BACKTEST_POOL_FILE), exist_ok=True)
        with open(BACKTEST_POOL_FILE, 'w', encoding='utf-8') as f:
            json.dump({'stocks': stocks, 'updated': datetime.now().strftime('%Y-%m-%d %H:%M:%S')}, f, ensure_ascii=False)
        return True
    except Exception as e:
        print(f"保存回测股票池失败: {e}")
        return False
...
@app.route('/api/bpool/update', methods=['POST'])
def update_backtest_pool():
    data = request.get_json()
    stocks = data.get('stocks', [])
    save_backtest_pool(stocks)
    return jsonify({
        'current_pool': stocks,
        'message': '回测股票池更新成功'
    })

@app.route('/api/bpool/add', methods=['POST'])
def add_backtest_stock():
    data = request.get_json()
    stock_code = data.get('stock_code')
    if not stock_code:
        return jsonify({'error': '请提供股票代码'}), 400
    ...
    if stock_code not in stocks:
        ...
        save_backtest_pool(stocks)
    return jsonify({'message': f'股票 {stock_code} 添加成功', 'current_pool': stocks})

@app.route('/api/bpool/remove', methods=['POST'])
def remove_backtest_stock():
    data = request.get_json()
    stock_code = data.get('stock_code')
    if not stock_code:
        return jsonify({'error': '请提供股票代码'}), 400
    ...
    if stock_code in stocks:
        ...
        save_backtest_pool(stocks)
    return jsonify({'message': f'股票 {stock_code} 移除成功', 'current_pool': stocks})

@app.route('/api/bpool/clear', methods=['POST'])
def clear_backtest_pool():
    save_backtest_pool([])
    return jsonify({'message': '回测股票池已清空', 'current_pool': []})
...
def get_general_settings():
    return jsonify({
        ...
        "defaultStrategy": "niu_huicai_atr"
    })
...
def save_general_settings():
    ...
    return jsonify({"message": "通用设置保存成功", "defaultStrategy": data.get('defaultStrategy', 'niu_huicai_atr')})
...
def get_market_status():
    ...
    try:
        ...
        try:
            ...
            lg = bs.login()
            print(f"baostock登录结果: error_code={lg.error_code}, error_msg={lg.error_msg}")
            if lg.error_code == '0':
                # 获取最近120天的沪深300数据
                end_date = datetime.now().strftime("%Y-%m-%d")
                start_date = (datetime.now() - pd.Timedelta(days=150)).strftime("%Y-%m-%d")
                
                print(f"查询沪深300数据: {start_date} 到 {end_date}")
                ...
                print(f"baostock查询结果: error_code={rs.error_code}, error_msg={rs.error_msg}")
                ...
                if rs.error_code == '0':
                    ...
                    if data_list:
                        ...
                        hs300_index = round(float(latest_data['close']), 2)
                        
                        # 计算120日均线（如果数据不足120天，使用可用数据计算）
                        if len(df) >= 120:
                            df['ma120'] = df['close'].rolling(window=120).mean()
                            ma120 = round(float(df.iloc[-1]['ma120']), 2)
                        else:
                            # 数据不足120天，使用60日均线
                        ...
                        if len(df) >= 7:
                            ...
                            ma120_values = [round(float(val), 2) for val in recent_df['ma120'].values]
                        else:
        ...
        result = {
            ...
            "chart_data": {
                "labels": ["01-20", "01-21", "01-22", "01-23", "01-24", "01-25", "01-26"],
                "hs300": [3950, 3960, 3970, 3980, 3990, 4000, 4000],
                "ma120": [3900, 3910, 3920, 3930, 3940, 3950, 3950]
            }
        }
        return jsonify(result)
...
# 修复后的调仓策略API
@app.route('/api/plan/adjustment_fixed', methods=['GET'])
def get_adjustment_strategy_fixed():
    from datetime import datetime
    try:
        position_file = get_position_file_path()
        positions = []
