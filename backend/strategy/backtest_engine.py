...
class BacktestEngine:
    ...
    def run_multi_stock_backtest(self, symbols, start_date, end_date, 
        ...
        try:
            ...
            stock_start = (datetime.strptime(start_date_fmt, '%Y-%m-%d') - timedelta(days=400)).strftime('%Y%m%d')
    ...
    def _backtest_single_stock(self, symbol, df, fast_period, slow_period,
        ...
        try:
            ...
            elif strategy_type == 'niu_huicai_atr':
                df = self._generate_bull_retracement_atr_signals(df)
            ...
            is_atr_strategy = (strategy_type == 'niu_huicai_atr')
            
            print(f"开始模拟交易: base_cash={base_cash}, 牛市资金={bull_cash}, 熊市资金={bear_cash}, is_atr_strategy={is_atr_strategy}")
            ...
            for i in range(len(df)):
                ...
                # 止盈止损检查（牛回踩ATR策略不使用固定止盈，只用ATR追踪止盈）
                if position > 0 and buy_price > 0:
                    profit_rate = (price - buy_price) / buy_price
                    
                    # 牛回踩ATR策略：跳过固定止盈，使用ATR追踪止盈
                    if not is_atr_strategy:
    ...
    def _generate_bull_retracement_atr_signals(self, df):
        """牛回踩ATR策略 - 基于ATR的动态追踪止盈"""
        df['ma20'] = df['close'].rolling(window=20).mean()
        df['ma250'] = df['close'].rolling(window=250).mean()
        df['v_ma5'] = df['volume'].rolling(window=5).mean()
        df['v_ma60'] = df['volume'].rolling(window=60).mean()
